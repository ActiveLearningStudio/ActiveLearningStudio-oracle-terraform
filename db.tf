     ...
     provisioner "remote-exec" {
         inline = [
          ...
          "sudo mkdir -p /mnt/DBData/pgadmin1-data",
           ...
           "sed -i 's/substitute-pgadmin-default-email/${var.pgadmin_email}/g' ~/curriki-db/.env.example",
           "sed -i 's/substitute-pgadmin-default-password/${var.pgadmin_password}/g' ~/curriki-db/.env.example",
           ...
           "sed -i 's/substitute-postgres-exposed-port/${var.postges_port}/g' ~/curriki-db/.env.example",
           "sed -i 's/substitute-pgadmin-exposed-port/${var.pgadmin_port}/g' ~/curriki-db/.env.example",
           "sed -i 's/substitute-phpmyadmin-exposed-port/${var.phpmyadmin_port}/g' ~/curriki-db/.env.example",
           "sed -i 's/substitute-postgres-database/${var.postgres_lrs_db}/g' ~/curriki-db/postgresscripts/traxdb.sql",
